  ...
  return (
    <div>
      <header className="mb-6">
        <h1 className="font-display text-4xl sm:text-5xl tracking-wider mb-1">CLASSEMENT</h1>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <Link key={r.id} href={`/player/${encodeURIComponent(r.username)}`}
                className={`card card-hover stagger-item !p-3 flex items-center gap-3 transition active:scale-[0.98] ${i === 0 ? "!border-neon/40 !bg-neon/5" : ""}`}>
                <div className="font-display text-2xl w-7 text-center text-gray-400">{medal(i)}</div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-base truncate">{r.username}</div>
                  <div className="text-[11px] text-gray-400 flex items-center gap-2 flex-wrap font-medium">
                ...
                <div className="text-right">
                  <div className="font-display text-3xl text-neon glow-text leading-none">
                    <CountUp end={r.points} />
                  </div>
      ...
      {rows.length > 0 && (
        <div className="hidden md:block card overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-gray-400 uppercase text-xs tracking-wider border-b border-border font-bold">
            ...
            <tbody>
              {rows.map((r, i) => {
                ...
                const color = e?.favorite_color || "#00ff87";
                return (
                  <tr key={r.id} className={`border-b border-border/50 hover:bg-bg/40 transition ${i === 0 ? "bg-neon/5" : ""}`}>
                    <td className="py-3 pr-2 font-display text-xl">{medal(i)}</td>
                    <td className="py-3 pr-2 font-bold">
                      <Link href={`/player/${encodeURIComponent(r.username)}`} className="hover:text-neon flex items-center gap-3">
                        <span className="inline-flex items-center justify-center w-9 h-9 rounded-full text-xl border-2"
                    ...
                    <td className="py-3 px-2 text-right text-gray-300 font-medium">{r.played}</td>
                    ...
                    <td className="py-3 pl-2 text-right text-neon font-display text-2xl">
                      <CountUp end={r.points} />
                    </td>
                  </tr>
                );
